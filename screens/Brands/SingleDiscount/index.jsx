...
import { fn_getSinglePostApi, fn_updatePostApi, API_URL } from "../../../api/api";
...
const SingleDiscountDetails = ({ route }) => {
    const toast = useToast();
    const navigation = useNavigation();
    const routeParams = useRoute();
    const discountId = routeParams.params?.discountId;
    ...
    // Fetch discount data on component mount
    useEffect(() => {
        if (discountId) {
            fetchDiscountData();
        } else {
    ...
    const fetchDiscountData = async () => {
        try {
            setLoading(true);
            const response = await fn_getSinglePostApi(discountId);
            if (response.status) {
                ...
                // Handle upload type and date/time
                if (data.uploadTime && data.uploadDate && data.uploadTime !== "" && data.uploadDate !== "") {
                    setUploadType("set");
                    setSelectedDate(data.uploadDate);
                    setSelectedTime(data.uploadTime);
                } else {
            ...
            toast.show("❌ Error loading discount data");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleUpdate = async () => {
        ...
        const response = await fn_updatePostApi(discountId, formData);
        setLoader(false);
